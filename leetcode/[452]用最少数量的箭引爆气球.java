...
//                return o1[1] - o2[1]; // 这样写在遇到[[-2147483646,-2147483645],[2147483646,2147483647]]时会出错
                if (o1[1] > o2[1]) {
                    return 1;
                } else if (o1[1] < o2[1]) {
                    return -1;
...
//                著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。
        ...
        int end = points[0][1]; // 第一个气球的end
        int count = 1; // 至少1支箭
        for (int i = 1; i < n; i++) {
            if (points[i][0] > end) {// 若区间无交叉，则需要新的箭，并更新end
                end = points[i][1];
                count++;
            }
            // 若区间有交叉(包括边界重合)，则不需要新的箭
        }
        return count;
    }
}
